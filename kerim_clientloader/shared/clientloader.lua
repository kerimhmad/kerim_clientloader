    ...
    if Resources[resource_name] ~= nil then
        if IsDuplicityVersion() then
            ...
            for k, v in ipairs(Resources[resource_name]) do
                local client_file = (LoadResourceFile(resource_name, v) or io.open(v, "r") or nil)
        ...
        elseif not IsDuplicityVersion() then
            if EnableInstantLoad then
                TriggerServerEvent(resource_name)
            elseif not EnableInstantLoad then
            ...
            RegisterNetEvent(trigger_event, function(client_files)
                ...
                for k, v in ipairs(client_files) do
                    ...
                    elseif not client_loaded then
                        ...
                        print(string.format("^1(!) ^0An error occurred while loading ^3%s^0!", v.file))
                    end
                end

                if success > 0 then
                    print(string.format("^2(!) ^0Script loaded successfully. (%s/%s)", success, success + errors))
                end
            end)
        end
    end
